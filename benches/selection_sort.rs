...
use sort_benchmarks::{generate_test_array, selection_sort_c, sort::selection_sort_rs};
...
fn selection_sort_bench(c: &mut Criterion) {
    ...
    for size in [10, 100, 1000, 10000].iter() {
        group.bench_with_input(BenchmarkId::new("Rust", size), size, |b, &size| {
            b.iter_batched(
                ...
                |mut data| selection_sort_rs(data.as_mut_slice()),
        ...
        group.bench_with_input(BenchmarkId::new("C", size), size, |b, &size| {
            b.iter_batched(
                || generate_test_array(size),
                |mut data| unsafe { selection_sort_c(data.as_mut_ptr(), data.len() as i32) },
                BatchSize::SmallInput,
            )
        });
    }
